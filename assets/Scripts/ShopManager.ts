...
/**
     * 即時掃描畫面上所有的方塊，找出沒有與 Core 相連的孤立方塊並讓它們掉落。
     * @param excludeNode 正在被玩家拖曳/拔起的方塊（掃描時必須無視它）
     */
    public checkAndDropUnconnectedParts(excludeNode: cc.Node = null) {
        let partLayer = this.getPartLayer();
        if (!partLayer) return;

        let allParts: cc.Node[] = [];
        let coreNode: cc.Node | null = null;

        // 1. 收集目前場上所有的有效方塊，並找出 Core
        for (let p of partLayer.children) {
            // 排除正在拖曳的方塊，以及綠色/紅色的提示框
            if (p === excludeNode || p.name === "placeHint") continue;

            allParts.push(p);

            // 使用你寫好的 isCoreNode 判斷
            if (this.isCoreNode(p)) {
                coreNode = p;
            }
        }
        ...
        // 2. 使用 BFS (廣度優先搜尋) 找出所有跟 Core 有相連的方塊
        let connected = new Set<cc.Node>();
        let queue: cc.Node[] = [coreNode];
        connected.add(coreNode);

        while (queue.length > 0) {
            let curr = queue.shift();
            // 計算目前這個方塊的網格座標 (跟你 Fight() 裡的邏輯一樣)
            let cx = Math.floor(curr.x / GRID.CELL_SIZE);
            let cy = Math.floor(curr.y / GRID.CELL_SIZE);

            // 檢查其他還沒被標記相連的方塊
            for (let p of allParts) {
                if (!connected.has(p)) {
                    let px = Math.floor(p.x / GRID.CELL_SIZE);
                    let py = Math.floor(p.y / GRID.CELL_SIZE);
        ...
        // 3. 把「沒有」被加進 connected 裡的方塊，通通判定為孤立並掉落！
        for (let p of allParts) {
            if (!connected.has(p)) {
                console.log(`[ShopManager] 方塊孤立，觸發掉落: ${p.name}`);
                this.dropPart(p);
            }
        }
    }
    ...
    /**
     * 觸發單一節點的物理掉落，並在 1.5 秒後銷毀
     */
    private dropPart(node: cc.Node) {
        // 1. 呼叫 Draggable 的重置物理 (恢復重力)
        let draggable = node.getComponent("Draggable") as any;
        if (draggable && typeof draggable.resetPhysics === "function") {
            draggable.resetPhysics();
        } else {
            // 防呆：手動開物理
            let rb = node.getComponent(cc.RigidBody);
            if (rb) {
                node.group = "default";
                (node.getComponents(cc.PhysicsCollider) as cc.PhysicsCollider[]).forEach(c => c.apply());
                rb.type = cc.RigidBodyType.Dynamic;
                rb.awake = true;
            }
        }

        // 2. 把它移出 PartLayer，丟到 Canvas 底下
        // 這樣它掉落的過程就不會干擾到我們下一次的網格判定
        let canvas = cc.find("Canvas");
        if (canvas) {
