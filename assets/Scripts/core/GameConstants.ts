...
export const PHYSICS = {
    GRAVITY_Y: -960,
    VELOCITY_ITERATIONS: 60,    // 約束求解速度迭代。調高 → 焊接在空中扭矩/碰撞下更硬、整車更不易變形（加硬關節）
...
// 著地/騰空偵測探測距離（空中旋轉改由統一 Box2D 的 AIRBOX 處理）
export const AIR = {
    // 狀態遲滯（避免空中/地面反覆抖動）：在空中用短探測判落地、在地面用長探測判起飛，兩者間是死區。
    // 規則 LAND_PROBE < TAKEOFF_PROBE；差值＝死區大小（越大越不易抖、但起降反應越鈍）。
    LAND_PROBE: 8,             // 【空中→地面】零件邊緣外再延伸多少 px 命中地面才算「落地」（短：要很貼地）
    TAKEOFF_PROBE: 40,         // 【地面→空中】零件邊緣外此距離內都沒地面才算「離地」（長：要明顯離地才接管空中）
    CONTACT_PROBE: 15,         // 「完全無接觸才進入空中模式」的多方向接觸偵測邊距（px）。愈大愈容易判定為接觸中（→ 愈難進入空中接管）
};

// Debug：按 P 切換，畫出每個零件的碰撞邊界與接觸探測射線，並用顏色標示騰空狀態。
// 綠=在地面、紅=騰空中。
export const DEBUG = {
    SHOW_BOUNDS: false,       // 初始是否開啟（遊戲中按 P 可切換）
};

// 統一 Box2D 的空中控制（只套用在玩家車）：車子永遠是動態體，騰空時做「重力縮放 + 對核心施扭矩旋轉」。
export const AIRBOX = {
    GRAVITY_SCALE: 0.625,  // 空中重力縮放（世界 -960 × 此 ≈ -600）。越小越飄、滯空越久；落地還原 1
    MAX_OMEGA: 360,        // 空中最大旋轉角速度（度/秒）。按住 A/D 轉到此速度為止；放開回 0。太大會散架就調小
    TORQUE_GAIN: 3000,     // (目標角速度 − 目前角速度) × 此 = 扭矩（P 控制）。越大到達越快、過衝越多
    MAX_TORQUE: 300000,    // 扭矩上限（壓在自動翻正 250000 同量級 → 焊接拉得住、不散架）。方向相反就翻 moveDir 符號
};
