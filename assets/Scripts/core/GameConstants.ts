...
export const AIRPHYS = {
    ...
    LAND_VEL_SCALE: 0.25,  // 落地交回 Box2D 時，灌入速度的縮放（0=落地即停、1=完整保留動量）。
                           // 調小避免「彈一下又進空中再落地→越彈越兇」；調大則落地保留較多衝勁
    RE_ENTER_WINDOW: 0.2,  // 落地後多少秒內算「剛落地」。此窗內若又重進空中，會夾掉地板彈出的向上速度，
                           // 擋「落地→被彈一下→重進空中→瞬間噴起來」。不擋接管（一騰空仍立刻吃空中輕重力＋邊界夾制）
    MAX_ENTER_SPEED: 2000, // 進空中初速上限（px/s，給很寬只擋退化尖刺）。即使有殘留彈出速度也不會誇張噴飛
};
